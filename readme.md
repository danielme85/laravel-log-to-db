...
<i>For optional MongoDB support you need to install the mongodb/laravel-mongodb package</i>
```
...
## Configuration
You will need to add an array under 'channels' in your `config/logging.php` for Log-to-DB like so:
```php
'database' => [
    'driver' => 'custom',
...
More info about some of these options: https://laravel.com/docs/logging#customizing-monolog-for-channels
...
the Laravel Log Facade. See https://laravel.com/docs/logging for more information.
```php
...
You can either add [closures](https://laravel.com/docs/eloquent#events-using-closures) on your custom application model mentioned above,
or add a [model observer](https://laravel.com/docs/eloquent#observers) for the default LogToDb models.
<br>
Create a observer:
```
<?php
...
```php
LogToDB::model()->removeOldestIfMoreThan(100);
```
